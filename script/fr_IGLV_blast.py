...
TB_all = tb
TB_all['has_sig'] = False
# ## take the seq from the genome
# save the sequences

TB_all.sstart = TB_all.sstart.astype(int)
TB_all.send = TB_all.send.astype(int)
...
for i in range(TB_all.shape[0]):
    ...
    if TB_all.seg_f.iloc[i]:
        ...
        seq_aa = str(seq_dict[TB_all.sacc.iloc[i].split('_')[0]][(TB_all.iloc[i].send-1):(TB_all.iloc[i].send-1) + 390].seq.reverse_complement().translate())
        seq_nr = str(seq_dict[TB_all.sacc.iloc[i].split('_')[0]][(TB_all.iloc[i].send-1):(TB_all.iloc[i].send-1) + 390].seq.reverse_complement())
    Seq_aa.append(f">{seq_id}\n{seq_aa}")
    Seq_nr.append(f">{seq_id}\n{seq_nr}")
    if "CACAGTG" in seq_nr and "AAAAA" in seq_nr:
        TB_all.has_sig.iloc[i] = True
        Func.append((seq_id, seq_aa))

TB_all.to_csv('result/PotentialLV.tsv', sep='\t') 
with open('result/PotentialLV_aa.fa', 'w') as f:
    f.write('\n'.join(Seq_aa))
with open('result/PotentialLV_nr.fa', 'w') as f:
    f.write('\n'.join(Seq_nr))
...
for i in range(TB_all.shape[0]):
    ...
    if "CACAGTG" in seq_nr and "AAAAA" in seq_nr:
        if "VYYC" in seq_aa:
             send = seq_nr[seq_aa.find('VYYC') * 3:].find('CACAGTG') + seq_aa.find('VYYC') * 3
             if "*" not in seq_aa[:int(send/3)]:
                 print(">"+seq_id)
                 print(seq_nr[:int(send)-1])
